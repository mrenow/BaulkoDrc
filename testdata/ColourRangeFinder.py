...
def mouse_click(event, x, y, flags, param):
    ...
    global lowerSliders, lowerColour, upperColour, imageStackedChangeColour
    if event == cv2.EVENT_LBUTTONDOWN:
        point = [x, y]
        if(lowerSliders):
            lowerColour = imageStackedChangeColour[y,x]
        else:
            upperColour = imageStackedChangeColour[y,x]
        cv2.setTrackbarPos('Channel 1 Upper', 'trackbars', max(upperColour[0],lowerColour[0]))
        cv2.setTrackbarPos('Channel 2 Upper', 'trackbars', max(upperColour[1],lowerColour[1]))
        cv2.setTrackbarPos('Channel 3 Upper', 'trackbars', max(upperColour[2],lowerColour[2]))
        cv2.setTrackbarPos('Channel 1 lower', 'trackbars', min(upperColour[0],lowerColour[0]))
        cv2.setTrackbarPos('Channel 2 lower', 'trackbars', min(upperColour[1],lowerColour[1]))
        cv2.setTrackbarPos('Channel 3 lower', 'trackbars', min(upperColour[2],lowerColour[2]))

        lowerSliders = not lowerSliders
